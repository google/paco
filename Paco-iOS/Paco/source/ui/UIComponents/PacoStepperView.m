...
- (void)setFormat:(NSString *)format {
  _format = format;
  self.valueLabel.text = [NSString stringWithFormat:format, (double)self.stepper.value];
  [self.valueLabel sizeToFit];
}

- (void)setValue:(NSNumber *)value {
  _value = value;
  self.stepper.value = [_value doubleValue];
  if (value && _format) {
    self.valueLabel.text = [NSString stringWithFormat:_format, [value doubleValue]];
  }
  [self.valueLabel sizeToFit];
}
...
- (BOOL)valueChangedInRange:(double)value {
...
- (void)valueChanged:(UIStepper *)stepper {
  ...
  [self.tableDelegate dataUpdated:self
                          rowData:[NSNumber numberWithDouble:stepper.value]
...
- (void)textFieldDidChange:(UITextField *)textField {
  self.value = [NSNumber numberWithDouble:textField.text.doubleValue];
  self.stepper.value = textField.text.doubleValue;
  [self layoutSubviews];
}

- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
  NSString *str = [textField.text stringByAppendingString:string];
  BOOL edit = [self valueChangedInRange:str.doubleValue];
