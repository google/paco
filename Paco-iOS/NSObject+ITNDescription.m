...
- (NSString *)itn_descriptionWithIndentation:(NSUInteger)indent
  ...
  NSMutableString *d =
      [NSMutableString stringWithFormat:@"<%@: %p>", self.class, self];
  ...
  for (Class class in [self itn_classes]) {
    ...
    for (unsigned int i = 0; i < count; i++) {
      ...
      if (ivar && strlen(ivar)) {
        ...
        for (NSUInteger i = 0; i < indent + ITN_INDENT; i++)
          [d appendString:@" "];
        ...
        if (value) {
          subdescription =
              [value itn_descriptionWithIndentation:(indent + ITN_INDENT)
                                              depth:(depth - 1)];
        }
...
- (NSString *)itn_descriptionWithIndentation:(NSUInteger)indent
                                       depth:(NSUInteger)depth {
  if (self == (id)kCFBooleanTrue || self == (id)kCFBooleanFalse) {
...
- (NSString *)itn_descriptionWithIndentation:(NSUInteger)indent
  ...
  NSMutableString *d = [NSMutableString
      stringWithFormat:@"%lu object%@", (unsigned long)self.count,
                       (self.count == 1 ? @"" : @"s")];
...
- (NSString *)itn_descriptionWithIndentation:(NSUInteger)indent
                                       depth:(NSUInteger)depth {
  NSMutableString *d = [NSMutableString
      stringWithFormat:@"%lu pair%@", (unsigned long)self.count,
                       (self.count == 1 ? @"" : @"s")];
  if (depth == 0 || self.count == 0) return d;  // Short description.
  ...
  for (id key in self) {
    ...
    NSString *keyDescription =
        [key itn_descriptionWithIndentation:(indent + ITN_INDENT)depth:0];
