 ...
 *
 */
public class UsageStatsBackfillJob {
  ...
  public void run() throws IOException {
    ...
    if (experimentList != null) {
      ...
      DateMidnight current = DateTime.parse("2016/03/20", TimeUtil.dateFormatter).toDateMidnight();
      
      List<ExperimentDAO> sublist = Lists.newArrayList();
      int i = 0;
      DateMidnight anyGivenSunday = oldestDate.plusDays(offset);      
      
      while (sublist.size() < experimentList.size() && anyGivenSunday.isBefore(current)) {
        ...
        runWeek(anyGivenSunday.toDateTime(), sublist, numberOfEvents, totalParticipantsJoined);
