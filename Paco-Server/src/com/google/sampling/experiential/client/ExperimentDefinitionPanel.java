...
// import java.text.SimpleDateFormat;       // PRIYA - for now
...
public class ExperimentDefinitionPanel extends Composite {
  ...
  // PRIYA - which panels need to be kept as data members?
  ...
  VerticalPanel leftMenuBar;
  ExperimentMetadataPanel metadataPanel;
  SignalMechanismChooserPanel signalPanel;
  InputsListPanel inputsListPanel;
  ExperimentCreationContentPanel contentPanel;

  Widget showingPanel;    // PRIYA
  ...
  public ExperimentDefinitionPanel(ExperimentDAO experiment, LoginInfo loginInfo, ExperimentListener listener) {
    ...
    leftMenuBar = new VerticalPanel();
    createLeftMenuBar();

    contentPanel = new ExperimentCreationContentPanel();
    mainPanel.add(contentPanel);

    metadataPanel = new ExperimentMetadataPanel(experiment, loginInfo, listener);
    contentPanel.setShowingView(metadataPanel);
  ...
  private void createLeftMenuBar() {
    ...
    labelMessage.setWidth("200");
    labelMessage.setText("Experiment Creation\n");

    leftMenuBar.add(labelMessage);

    TreeItem showDescription = new TreeItem("Experiment Description");
    TreeItem experimentFinishing = new TreeItem("Experiment Finishing");
    //    Tree showDescriptionTree = new Tree();
    //    showDescriptionTree.addItem(showDescription);
    //    leftMenuBar.add(showDescriptionTree);

    TreeItem inputGroups = new TreeItem("Input Groups");

    TreeItem inputGroup1 = new TreeItem("Input Group 1");

    TreeItem showSchedule = new TreeItem("Experiment Schedule");
    TreeItem showInputs = new TreeItem("Experiment Inputs");

    inputGroup1.addItem(showSchedule);
    inputGroup1.addItem(showInputs);

    inputGroups.addItem(inputGroup1);

    Tree inputGroupTree = new Tree();
    inputGroupTree.addItem(showDescription);
    inputGroupTree.addItem(inputGroups);
    inputGroupTree.addItem(experimentFinishing);
    
    inputGroups.setState(true);
    inputGroup1.setState(true);

    inputGroupTree.addSelectionHandler(new SelectionHandler<TreeItem>() {
      @Override
      public void onSelection(SelectionEvent<TreeItem> event) {
        String selectedButtonTitle = event.getSelectedItem().getText();
        if (selectedButtonTitle.equals("Experiment Description")) {
          showExperimentDescription();
        } else if (selectedButtonTitle.equals("Experiment Schedule")) {
          showExperimentSchedule();
        } else if (selectedButtonTitle.equals("Experiment Inputs")) {
          showExperimentInputs();
        } else if (selectedButtonTitle.equals("Experiment Finishing")) {
          showExperimentForm();
        }
      }
    });

    //    inputGroupTree.addSelectionHandler(new SelectionHandler() {
    //      @Override
  ...
  leftMenuBar.add(inputGroupTree);


  //    Tree experimentFinishingTree = new Tree();
  //    experimentFinishingTree.addItem(experimentFinishing);
  //    leftMenuBar.add(experimentFinishingTree);

  //    Button showDescription = new Button("Experiment Description"); // PRIYA
  //    showDescription.addClickListener(new ClickListener() {
  //      
  //      @Override
  ...
  //      }
  //    });
  //    leftMenuBar.add(showDescription);
  //    
  //    DisclosurePanel inputGroups = new DisclosurePanel("Input Groups");
  //    
  //    DisclosurePanel firstInputGroup = new DisclosurePanel("Input Group 1");
  //    VerticalPanel inputGroupHolder = new VerticalPanel();
  //    Button showSchedule = new Button("Experiment Schedule"); // PRIYA
  //    showSchedule.addClickListener(new ClickListener() {
  //      
  //      @Override
  //      public void onClick(Widget sender) {
  //        showExperimentSchedule();
  //      }
  //    });
  //    inputGroupHolder.add(showSchedule);
  //    Button showInputs = new Button("Experiment Inputs"); // PRIYA
  //    showInputs.addClickListener(new ClickListener() {
  //      
  //      @Override
  //      public void onClick(Widget sender) {
  //        showExperimentInputs();
  //      }
  //    });
  //    inputGroupHolder.add(showInputs);
  //    firstInputGroup.add(inputGroupHolder);
  //    leftMenuBar.add(firstInputGroup);


  //    Button showForm = new Button("Experiment Finishing"); // PRIYA
  //    showForm.addClickListener(new ClickListener() {
  //      
  //      @Override
  //      public void onClick(Widget sender) {
  //        showExperimentForm();
...
private void showExperimentDescription() {
  if (showingPanel.equals(metadataPanel)) {
    saveMetadataInfo();
  } else if (showingPanel.equals(formPanel)){
    // Do nothing for now. PRIYA
    saveFormInfo();   // RENAME
  }
  contentPanel.setShowingView(metadataPanel);
  showingPanel = metadataPanel;
...
private void showExperimentSchedule() {
  ...
  contentPanel.setShowingView(signalPanel);
...
private void showExperimentInputs() {
  ...
  contentPanel.setShowingView(inputsListPanel);
  showingPanel = inputsListPanel;
}

private void showExperimentForm() {
  if (showingPanel.equals(metadataPanel)) {
    saveMetadataInfo();
  } else if (showingPanel.equals(formPanel)){
    saveFormInfo();   // RENAME
  }
  contentPanel.setShowingView(formPanel);
  showingPanel = formPanel;
}
...
private void createExperimentForm() {
  ...
  //
  // PanelPair informedConsentPanelPair =
  // createInformedConsentPanel(experiment);
...
// Integer.toString(experiment.getVersion() == null ? 0 :
// experiment.getVersion()));
// }
//
//
// private PanelPair createDescriptionPanel(ExperimentDAO experiment) {
// return createFormArea(myConstants.experimentDescription(),
// experiment.getDescription(), 75, "100");
// }
//
// private PanelPair createCreatorPanel(ExperimentDAO experiment) {
// return createDisplayLine(myConstants.experimentCreator(),
// experiment.getCreator() != null ? experiment.getCreator() :
// loginInfo.getEmailAddress());
// }
//
// private PanelPair createInformedConsentPanel(ExperimentDAO experiment) {
// return createFormArea(myConstants.informedConsent(),
// experiment.getInformedConsentForm(), 100, "200");
// }

// private HTML createInputsHeader() {
// HTML questionsPrompt = new HTML("<h2>" +
// myConstants.enterAtLeastOneQuestion() + "</h2>");
...
// experiment.getFixedDuration(), experiment.getStartDate(),
...
private void createButtonPanel(ExperimentDAO experiment) {
  ...
  buttonPanel.add(createNextButton(experiment));
  buttonPanel.add(createSubmitButton(experiment));
  buttonPanel.add(createCancelButton());
  // formPanel.add(buttonPanel);
  mainPanel.add(buttonPanel); // PRIYA
}
...
// private DisclosurePanel createAdminDisclosurePanel(ExperimentDAO
// experiment) {
// final DisclosurePanel adminPanel = new DisclosurePanel();
// final DisclosurePanelHeader closedHeaderWidget =
// new DisclosurePanelHeader(false, "<b>" +
// myConstants.clickToEditAdministrators() + "</b>");
// final DisclosurePanelHeader openHeaderWidget =
// new DisclosurePanelHeader(true, "<b>" +
// myConstants.clickToCloseAdministratorEditor() + "</b>");
...
private Widget createNextButton(final ExperimentDAO experiment) {
...
private void setPublishingOn(ExperimentDAO experiment) {      // PRIYA - need to set checkbox and stuff separately
