...
public class EventMatcherTest extends TestCase {
  ...
  protected void setUp() throws Exception {
    ...
    Date responseTime = new DateTime(2009, 8, 31, 0, 0).toDate();
    Date when = new DateTime(2009, 9, 1, 0, 0).toDate();
    events.add(new Event("bobevans@google.com",
        null, null, when, "test", null, newHashSet, true, "1", "foo", 1, responseTime, responseTime, null, null));
  ...
  public void testMatchDateRange() throws Exception {
    query.add(new Query("date_range", "2009/08/31-2009/09/02"));
    List<Event> results = eventMatcher.matchingEvents(events, query);
    assertEquals(1, results.size());
  }

  public void testMatchDateRangeOneDate() throws Exception {
    query.add(new Query("date_range", "2009/09/01"));
    List<Event> results = eventMatcher.matchingEvents(events, query);
    assertEquals(1, results.size());
  }
