...
public class RuntimePermissionMonitorService extends AccessibilityService {
  ...
  public void onAccessibilityEvent(AccessibilityEvent accessibilityEvent) {
    ...
    switch (eventType) {
      case AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED:
        ...
        Log.debug("New accessibility event: window state changed (we are capturing this)");
        if (isAppPermissionsScreen(accessibilityEvent)) {
          // Find the package name in this view, and store it for future use
          Log.debug("We seem to be inside the app permissions screen");
          ...
          Log.debug("We seem to be inside the screen showing apps for a permission");
          ...
          Log.debug("We seem to be inside a runtime permissions dialog");
          ...
          Log.debug("Ignoring window state changed accessibility event, since it was not a permission settings screen or a permissions dialog.");
        }
        break;
      ...
      case AccessibilityEvent.TYPE_ANNOUNCEMENT:
        ...
        Log.debug("This might be a changed permissions dialog, try to extract info");
      ...
      case AccessibilityEvent.TYPE_VIEW_CLICKED:
        ...
        if (isPermissionsDialogAction(accessibilityEvent.getSource())) {
          Log.debug("Action taken in permissions dialog");
          ...
          Log.debug("Action taken in permission settings activity");
          processPermissionConfigurationChange(accessibilityEvent);
        } else {
          Log.debug("Ignoring TYPE_VIEW_CLICKED, since it was not in a permission dialog or a settings screen.");
        }
        break;
    }
  }
    ...
    for (CharSequence eventSubText : eventText) {
      ...
      if (permissionMatcher.find()) {
        ...
        if (permissionString == null) {
          Log.debug("Unknown permission string encountered: " + permissionText);
          permissionString = permissionText;
        }
        addEncounteredPermission(permissionString, permissionMatcher.group(1));
      } else {
        Log.debug("Could not extract any information from string " + eventSubText);
      }
    }
  }
  ...
  private void extractInformationFromAppListingForPermission(AccessibilityNodeInfo rootNodeInfo) {
    ...
    Log.debug("We failed to extract the permission string from the settings screen");
  }
  ...
  private boolean addEncounteredPermission(CharSequence permission, CharSequence appName) {
    ...
    if (lastPermissionRequest != null &&
      ...
      Log.debug("Not adding the same permisison request twice!");
      return false;
    }
    ...
    Log.debug("Added previously handled permission " + permission.toString());
    return true;
  }
  ...
  private void setCurrentlyHandledAppPackageNames(ArrayList<String> packageNames) {
    ...
    Log.debug("Set 'currently handled package names' to " + currentlyHandledAppPackageNames.toString());
  }
  ...
  private void setCurrentlyHandledAppPackageName(CharSequence packageName) {
    ...
    try {
      ...
      Log.debug("Also set app name to " + currentlyHandledAppName);
    } catch (PackageManager.NameNotFoundException e) {
      Log.debug("Could not find app info for package " + packageName);
    }
  }
  ...
  private void setCurrentlyHandledAppName(CharSequence appName) {
    ...
    Log.debug("Set 'currently handled app name' to " + currentlyHandledAppName);
  ...
  private void setCurrentlyHandledPermission(String permission) {
    ...
    Log.debug("Set 'currently handled permission' to " + currentlyHandledPermission);
  }
  ...
  private void processPermissionDialogAction(AccessibilityNodeInfo nodeInfo) {
    if (previouslyEncounteredPermissionRequests.size() < 1) {
      Log.debug("We got a dialog action on a permission request, but we never saw" +
    ...
    while (encounteredPermission.getTimestamp() < System.currentTimeMillis() - PERMISSION_REQUEST_HISTORY_MILLIS &&
      ...
      Log.debug("Not considering permission request " +
  ...
  protected void onServiceConnected() {
    ...
    if (!Locale.getDefault().getISO3Language().equals(Locale.ENGLISH.getISO3Language())) {
      ...
      Log.debug("Detected locale is " + Locale.getDefault().toString() +
